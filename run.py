...
class FaceDetector:
    ...
    def detect_face(self, img, scaleFactor):
        ...
        face_cascade = cv2.CascadeClassifier(
            'opencv-files/haarcascades/haarcascade_frontalface_alt2.xml')
        faces = face_cascade.detectMultiScale(
            gray, scaleFactor=scaleFactor, minNeighbors=4)
...
class FaceRecognitor:
    ...
    def __init__(self, configPath):
        ...
        self.faces, self.labels, self.efaces = self.prepare_training_data(
            "training-data")
    ...
    def prepare_training_data(self, data_folder_path):
        ...
        cv2.waitKey(1)
        cv2.destroyAllWindows()
        print("[*] Mean faces: ", np.mean(np.asarray(sumFace[0])),
              np.mean(np.asarray(sumFace[1])))
    ...
    def test(self):
        ...
        for i in range(1, 16):
            ...
            file.write(
                str(i)+"\tEigen\t"+validate[0]+"\t" + str(result1[1]) + "\t" + result1[2] + "\n")
            file2.write(
                str(i)+"\tFisher\t"+validate[1]+"\t" + str(result2[1]) + "\t" + result2[2] + "\n")
            file3.write(
                str(i)+"\tLBPH\t"+validate[2]+"\t" + str(result3[1]) + "\t" + result3[2] + "\n")
